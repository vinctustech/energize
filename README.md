...
[![Version](https://img.shields.io/badge/latest_release-v0.13_snapshot_3-orange.svg)](https://github.com/vinctustech/energize/releases/tag/v0.13_snapshot_3)
...
- Type `java -jar energize-0.13_snapshot_3.jar --help` for executable options
...
If you just want to download the executable so that you can have an API server for your project or use the REPL, you can download it from [here](https://dl.bintray.com/edadma/generic/energize-0.13_snapshot_3.jar). *You do not need* the Scala library for it to work because the JAR already contains all dependencies. You just need Java 8+ installed.

Run it as a normal Java executable JAR with the command `java -jar energize-0.13_snapshot_3.jar <config>` in the folder where you downloaded the file, where *config* is the name of the `.energize` file (without the file extension) that defines your API.
...
Use the following definition to use *Energize* in your Maven project:
	...
	<dependency>
	  ...
	  <version>0.13_snapshot_3</version>
	</dependency>
...
Add the following to your `build.sbt` file to use *Energize* in your SBT project:
    ...
    libraryDependencies += "xyz.hyperreal" %% "energize" % "0.13_snapshot_3"
...
Now, on the command line in the example folder, start the server with the command
    ...
    java -jar energize-0.13_snapshot_3.jar todo
...
To start the REPL (read, execute, print, loop), type the following command (while in the same folder where the executable was placed)
	...
	java -cp energize-0.13_snapshot_3.jar xyz.hyperreal.energize.REPLMain
