...
//    """.trim.stripMargin
        ...
        |
        |todo.insert( {name: "first \"thing\"", status: 1} )
        |todo.insert( {name: 'second thing', status: 2} )
      """.stripMargin

  val pro = Definition.define( src, c, s, d, key )

  println( pro.process( "GET", "/templates/404", new SimpleMessage("Host" -> "example.com:80"), null, null ) )
