...
//	}
	...
	private def configure( ast: SourceAST, connection: Connection, statement: Statement, db: Database, key: String, internal: Boolean ): Environment = {
		...
		def interpretDefinitions( ast: AST ): Unit =
			ast match {
				...
				case TableDefinition( protection, pos, name, base, columns, resource ) =>
					...
					tables(db.desensitize( name )) = Table( name, cols map {case (_, cinfo) => cinfo} toList, cols.toMap, resource, base, mtm, null )
		...
		if (!internal) {
			...
			for ((k, v) <- e.tables)
				tables get k match {
					case Some( Table(name, columns, columnMap, _, _, mtm, _ )) =>
						tables(k) = Table(name, v.columns ++ columns, v.columnMap ++ columnMap, v.resource, v.base, v.mtm || mtm, null )
					case None => tables(k) = v
				}

			tables.values foreach {
				case Table( _, columns, _, _, _, _, _ ) =>
					columns foreach {
...
//				println( db.create(sorted) )
			...
			tables.values foreach {
				case t@Table( name, cols, _, _, _, _, _ ) =>
